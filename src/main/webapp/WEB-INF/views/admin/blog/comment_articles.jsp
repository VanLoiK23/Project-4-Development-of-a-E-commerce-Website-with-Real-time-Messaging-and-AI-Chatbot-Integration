...
<meta name="stylesheet"
	content='
		<link rel="stylesheet"
			...
			<link rel="stylesheet"
			href="<c:url value='/template/css/style/commentArticle.css' />" />
			' />

<meta name="scripts"
	content='
	        	<script src="<c:url value='/template/js/quan_li_comment_articles.js' />"></script>	    
	    ' />
...
<body>
	...
	<div class="main-content">
		...
		<form
			action="<c:url value='/quan-tri/QuanlicommentArticles_controller/danh-sach'/>"
											...
											<c:forEach var="item" items="${model.listResult}"
												...
												<tr style="color: black">
													...
													<td><c:forEach var="j" begin="1" end="5">
															<c:choose>
																<c:when test="${j <= item.rating}">
																	<i style="color: #FFD700;" class="fa-solid fa-star"></i>
																</c:when>
																<c:when test="${j - 0.5 <= item.rating&& j > item.rating}">
													...
													<td>
														<button type="button" onclick='show(${item.id})' type="button">
															<a style='color: white'
																href='http://localhost:8080/Spring-mvc/quan-tri/QuanlicommentArticles_controller/show_content/${item.id }'>
																<i class='fa-solid fa-eye'></i>
															</a>
														</button>
														<button type="button" onclick='Delete(${item.id});' type="button">
