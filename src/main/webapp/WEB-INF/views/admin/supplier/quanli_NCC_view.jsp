...
<meta name="scripts"
	content='
	        	<script src="<c:url value='/template/js/quan_li_ncc.js' />"></script>
	    ' />
...
<body>
	...
	<div class="main-content">
		...
		<%@ include file="add_NCC.jsp"%>
		<%@ include file="edit_NCC.jsp"%>
		...
		<form action="<c:url value='/quan-tri/supplier/danh-sach'/>"
			...
			<div class="p-4">
				<button type="button" class="bt-trash">
					<a style="color: white;"
						href="<c:url value='/quan-tri/supplier/trash'/>"> <i
						...
						<div class="table-section">
							<table>
								...
								<tr>
									...
									<th style="width: 170px;">Tên nhà cung cấp</th>
								...
								<tbody id="ds">
									<c:forEach var="item" items="${model.listResult}" varStatus="i">
										...
										<tr style="color: black">
											...
											<td
												style='white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>${item.tenNhaCungCap}</td>
											<td
												style='white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>${item.diaChi}</td>
											<td
												style='white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>${item.email}</td>

											<td>${item.soDienThoai}</td>

											<c:url var="disable" value="/quan-tri/supplier/status">
												<c:param name="type" value="disable" />
												<c:param name="id" value="${item.id}" />
												<c:param name="page" value="${model.page}" />
											</c:url>

											<c:url var="active" value="/quan-tri/supplier/status">
