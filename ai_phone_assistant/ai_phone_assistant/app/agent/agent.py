...
def get_history_address_from_api(user_identifier: str):
    ...
    api_url = f"http://localhost:8080/Spring-mvc/khach-hang/getInfoShipping?phone={user_identifier}"
...
def get_shipping_history(user_id: str) -> str:
    try:
        ...
        # Format dữ liệu thành chuỗi văn bản dễ đọc cho AI
        result_text = "Các thông tin giao hàng cũ tìm thấy:\n"
        for i, addr in enumerate(addresses, 1):
            result_text += f"[{i}] {addr['street_name']}, {addr['district']}, {addr['city']} (Người nhận: {addr['hovaten']}, Email: {addr['email']}, Số điện thoại: {addr['sodienthoai']})\n"
...
--- 🚨 LUẬT BẤT KHẢ KHÁNG (BẮT BUỘC THỰC HIỆN) 🚨 ---
Mọi câu hỏi của khách hàng liên quan đến sản phẩm (ví dụ: "có bán không", "còn hàng không", "giá bao nhiêu", "có [Tên Sản Phẩm] không", "đưa hình")...
=> BẠN BẮT BUỘC PHẢI GỌI TOOL `product_overview` HOẶC `product_detail_card` ĐỂ KIỂM TRA KHO HÀNG THỰC TẾ.
=> TUYỆT ĐỐI KHÔNG ĐƯỢC kết thúc hội thoại (Finished chain) mà chưa gọi tool nào nếu khách đang hỏi về sản phẩm.
...
* TRƯỜNG HỢP 2: Khách chỉ đang XEM/HỎI THÔNG TIN:
    - PHẢI TRẢ RA NGUYÊN VĂN JSON từ tool.

===============================
🔒 CHẾ ĐỘ KHÓA JSON (ANTI-THINKING MODE)
===============================
Khi bạn quyết định rằng cần trả JSON nguyên văn:
- Bạn PHẢI trả đúng CHUỖI JSON từ tool.
- NGAY SAU KHI trả JSON, bạn PHẢI DỪNG SUY NGHĨ HOÀN TOÀN.
- KHÔNG ĐƯỢC sinh thêm chữ, giải thích, mô tả, hoặc câu nào phía sau.
- KHÔNG ĐƯỢC tạo JSON khác.
- KHÔNG ĐƯỢC tự viết JSON.
- Chỉ được *echo lại* JSON nhận từ tool.
...
BƯỚC 3: THU THẬP VÀ XÁC NHẬN THÔNG TIN GIAO HÀNG (QUAN TRỌNG)
       ...
       2. Xử lý kết quả từ tool:
          - TRƯỜNG HỢP A (Có địa chỉ cũ):
            + Liệt kê các địa chỉ đó ra (Ví dụ: "Em thấy anh có địa chỉ cũ tại [Địa chỉ 1] và [Địa chỉ 2]").
            + Hỏi khách: "Anh/chị muốn giao đến địa chỉ nào ở trên hay dùng địa chỉ mới ạ?".
